...
void Settings::Load(SettingsInterface& si)
{
  ...
  cpu_execution_mode =
    ParseCPUExecutionMode(
      si.GetStringValue("CPU", "ExecutionMode2", GetCPUExecutionModeName(DEFAULT_CPU_EXECUTION_MODE)).c_str())
  ...
  cpu_recompiler_memory_exceptions = si.GetBoolValue("CPU", "RecompilerMemoryExceptions2", true);
  ...
  display_linear_filtering = si.GetBoolValue("Display", "LinearFiltering2", false);
  ...
  video_sync_enabled = si.GetBoolValue("Display", "VSync2", DEFAULT_VSYNC_VALUE);
  display_post_process_chain = si.GetStringValue("Display", "PostProcessChain", "");
  display_max_fps = si.GetFloatValue("Display", "MaxFPS2", DEFAULT_DISPLAY_MAX_FPS);
  ...
  bios_patch_fast_boot = si.GetBoolValue("BIOS", "PatchFastBoot2", DEFAULT_FAST_BOOT_VALUE);
#if _M_ARM || defined(_M_ARM64) 
  touch_enable = si.GetBoolValue("TouchController", "TouchEnable", true);
#else
  touch_enable = si.GetBoolValue("TouchController", "TouchEnable", false);
#endif

  sensors_enable = si.GetBoolValue("Sensors", "SensorsEnable", false);
  sensors_up = si.GetBoolValue("Sensors", "SensorsUp", false);
  sensors_down = si.GetBoolValue("Sensors", "SensorsDown", false);
  sensors_sens = si.GetFloatValue("Sensors", "SensorsSens", 1.5f);

  touch_analog = si.GetBoolValue("TouchController", "TouchAnalog", false);
  touch_log = si.GetBoolValue("TouchController", "TouchLog", false);
  touch_hide_dpad = si.GetBoolValue("TouchController", "TouchDPad", false);
  touch_show_lr3 = si.GetBoolValue("TouchController", "TouchL3R3", false);
  touch_action_extra = si.GetBoolValue("TouchController", "TouchActionsExtra", true);
  touch_dpad_extra = si.GetBoolValue("TouchController", "TouchDPadExtra", true);
  touch_force = si.GetBoolValue("TouchController", "TouchForce", false);

  shader_compatibility = static_cast<u8>(si.GetIntValue("Backend", "ShaderLang", 0));
  shader_opt = static_cast<u8>(si.GetIntValue("Backend", "ShaderOpt", 0));
  touch_opacity = static_cast<u8>(si.GetIntValue("TouchController", "TouchOpacity", 30));
  touch_port = static_cast<u8>(si.GetIntValue("TouchController", "TouchPort", 1));
  touch_scale = static_cast<float>(si.GetFloatValue("TouchController", "TouchScale", 1.0f));
  touch_scalep = static_cast<float>(si.GetFloatValue("TouchController", "TouchScaleP", 1.0f));
  touch_analog_s = static_cast<float>(si.GetFloatValue("TouchController", "TouchAnalogS", 1.0f));
  touch_analog_sc = static_cast<float>(si.GetFloatValue("TouchController", "TouchAnalogSC", 1.5f));
  touch_sens = static_cast<float>(si.GetFloatValue("TouchController", "TouchSens", 8.0f));

  native_event = si.GetBoolValue("GamePad", "NativeKeyEvents", false);
  native_ui = si.GetBoolValue("GamePad", "NativeKeyEventsUI", true);
  gaming_event = si.GetBoolValue("GamePad", "GamingKeyEvents", false);
  gaming_ui = si.GetBoolValue("GamePad", "GamingKeyEventsUI", true);
  gaming_quick = si.GetBoolValue("GamePad", "GamingQuickMenu", true);
  gaming_map = si.GetBoolValue("GamePad", "GamingMap", false);
  native_port = static_cast<u8>(si.GetIntValue("GamePad", "NativePort", 1));
  buffer_count = static_cast<u8>(si.GetIntValue("Backend", "BufferCount", 3));
  max_frames_to_run = static_cast<u8>(si.GetIntValue("Backend", "MaxFrames", 2));
  gaming_custom_cross = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapCross", 0));
  gaming_custom_circle = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapCircle", 1));
  gaming_custom_square = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapSquare", 2));
  gaming_custom_train = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapTrain", 3));
  gaming_custom_l1 = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapL1", 4));
  gaming_custom_r1 = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapR1", 5));
  gaming_custom_up = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapUp", 6));
  gaming_custom_down = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapDown", 7));
  gaming_custom_left = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapLeft", 8));
  gaming_custom_right = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapRight", 9));
  gaming_custom_l2 = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapL2", 10));
  gaming_custom_r2 = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapR2", 11));
  gaming_custom_l3 = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapL3", 12));
  gaming_custom_r3 = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapR3", 13));
  gaming_custom_start = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapStart", 14));
  gaming_custom_select = static_cast<u32>(si.GetIntValue("GamePad", "CustomMapSelect", 15));
...
void Settings::Save(SettingsInterface& si) const
{
  ...
  si.SetStringValue("CPU", "ExecutionMode2", GetCPUExecutionModeName(cpu_execution_mode));
  ...
  si.SetBoolValue("CPU", "RecompilerMemoryExceptions2", cpu_recompiler_memory_exceptions);
  ...
  si.SetBoolValue("Display", "LinearFiltering2", display_linear_filtering);
  ...
  si.SetBoolValue("Display", "VSync2", video_sync_enabled);
  ...
  si.SetFloatValue("Display", "MaxFPS2", display_max_fps);
  ...
  si.SetBoolValue("BIOS", "PatchFastBoot2", bios_patch_fast_boot);

  si.SetBoolValue("Backend", "SafeMemory", safe_memory);
  si.SetBoolValue("Backend", "CacheShader", cache_shader);
  si.SetBoolValue("Backend", "D3D93Comp", d3d93_compatibility);
  si.SetBoolValue("Backend", "D3D93Note", d3dnotice);
  si.SetBoolValue("Backend", "ForceLandscape", forceLandscape);
  si.SetBoolValue("Sensors", "SensorsEnable", sensors_enable);
  si.SetFloatValue("Sensors", "SensorsSens", sensors_sens);
  si.SetBoolValue("Sensors", "SensorsUp", sensors_up);
  si.SetBoolValue("Sensors", "SensorsDown", sensors_down);
  si.SetBoolValue("TouchController", "TouchEnable", touch_enable);
  si.SetBoolValue("TouchController", "TouchAnalog", touch_analog);
  si.SetBoolValue("TouchController", "TouchLog", touch_log);
  si.SetBoolValue("TouchController", "TouchDPad", touch_hide_dpad);
  si.SetBoolValue("TouchController", "TouchActionsExtra", touch_action_extra);
  si.SetBoolValue("TouchController", "TouchDPadExtra", touch_dpad_extra);
  si.SetBoolValue("TouchController", "TouchL3R3", touch_show_lr3);
  si.SetBoolValue("TouchController", "TouchForce", touch_force);
  si.SetIntValue("TouchController", "TouchOpacity", touch_opacity);
  si.SetIntValue("Backend", "ShaderLang", shader_compatibility);
  si.SetIntValue("Backend", "ShaderOpt", shader_opt);
  si.SetIntValue("TouchController", "TouchPort", touch_port);
  si.SetFloatValue("TouchController", "TouchScale", touch_scale);
  si.SetFloatValue("TouchController", "TouchScaleP", touch_scalep);
  si.SetFloatValue("TouchController", "TouchAnalogS", touch_analog_s);
  si.SetFloatValue("TouchController", "TouchAnalogSC", touch_analog_sc);
  si.SetFloatValue("TouchController", "TouchSens", touch_sens);

  si.SetIntValue("GamePad", "NativePort", native_port);
  si.SetIntValue("Backend", "BufferCount", buffer_count);
  si.SetIntValue("Backend", "MaxFrames", max_frames_to_run);
  si.SetBoolValue("GamePad", "NativeKeyEvents", native_event);
  si.SetBoolValue("GamePad", "GamingMap", gaming_map);
  si.SetBoolValue("GamePad", "NativeKeyEventsUI", native_ui);
  si.SetBoolValue("GamePad", "GamingKeyEvents", gaming_event);
  si.SetBoolValue("GamePad", "GamingKeyEventsUI", gaming_ui);
  si.SetBoolValue("GamePad", "GamingQuickMenu", gaming_quick);

  si.SetIntValue("GamePad", "CustomMapCross", gaming_custom_cross);
  si.SetIntValue("GamePad", "CustomMapCircle", gaming_custom_circle);
  si.SetIntValue("GamePad", "CustomMapSquare", gaming_custom_square);
  si.SetIntValue("GamePad", "CustomMapTrain", gaming_custom_train);
  si.SetIntValue("GamePad", "CustomMapL1", gaming_custom_l1);
  si.SetIntValue("GamePad", "CustomMapR1", gaming_custom_r1);
  si.SetIntValue("GamePad", "CustomMapUp", gaming_custom_up);
  si.SetIntValue("GamePad", "CustomMapDown", gaming_custom_down);
  si.SetIntValue("GamePad", "CustomMapLeft", gaming_custom_left);
  si.SetIntValue("GamePad", "CustomMapRight", gaming_custom_right);
  si.SetIntValue("GamePad", "CustomMapL2", gaming_custom_l2);
  si.SetIntValue("GamePad", "CustomMapR2", gaming_custom_r2);
  si.SetIntValue("GamePad", "CustomMapL3", gaming_custom_l3);
  si.SetIntValue("GamePad", "CustomMapR3", gaming_custom_r3);
  si.SetIntValue("GamePad", "CustomMapStart", gaming_custom_start);
  si.SetIntValue("GamePad", "CustomMapSelect", gaming_custom_select);
...
static std::array<const char*, 3> s_cpu_execution_mode_display_names = {
  {TRANSLATABLE("CPUExecutionMode", "Interpreter (Slowest)"),
   ...
   TRANSLATABLE("CPUExecutionMode", "Recompiler (Broken!)")}};
#else
static std::array<const char*, 3> s_cpu_execution_mode_display_names = {
  {TRANSLATABLE("CPUExecutionMode", "Interpreter (Slowest)"),
   TRANSLATABLE("CPUExecutionMode", "Cached Interpreter (Faster)"),
...
static constexpr auto s_gpu_renderer_names = make_array("D3D11", "Software");
static constexpr auto s_gpu_renderer_display_names =
  make_array(TRANSLATABLE("GPURenderer", "Hardware"), TRANSLATABLE("GPURenderer", "Software"));
