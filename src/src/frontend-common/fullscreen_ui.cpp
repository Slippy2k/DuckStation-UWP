...
#endif
#include <duckstation-uwp/UWP2Win32/LaunchItem.h>

static constexpr float LAYOUT_MAIN_MENU_BAR_SIZE = 20.0f; // Should be DPI scaled, not layout scaled!
extern bool isLandscape;
extern std::string GetPreviewPath(std::string path);
extern std::string appTitle;
extern void updateButtonsMap();
extern const char* GetKeyCodeName(int key_code);
extern int outLevelIndex;
...
extern void ResetEmptyList();
extern std::string getCurrentStorage();
extern std::string GetLocalFolder();
extern bool AddDataToLocalSettings(std::string key, std::string data, bool replace);
extern bool checkAccess(std::string path);
extern int browseFolderCaller;
bool s_quick_menu_was_open = false;
bool s_compatibility_window_open = false;
bool s_restart_window_open = false;
bool called = false;
extern bool isLevel93;
extern bool forcedByDeviceCreate;
...
void Render()
{
  ...
  if (s_compatibility_window_open)
    DrawCompatibilityWindow();

  if (s_restart_window_open)
    DrawRestartWindow();
...
{
  std::string currentStorage = getCurrentStorage();
  if (currentStorage.empty())
  {
    currentStorage = GetLocalFolder();
  }
  auto callback = [](const std::string& path) {
    std::string curr = getCurrentStorage();
    if (!path.empty() && curr != path)
    {
      bool isAccessible = false;
      try
      {
        isAccessible = checkAccess(path);
      }
      catch (...)
      {
      }
      if (isAccessible)
      {
        ...
        ClearImGuiFocus();
        CloseFileSelector();
        s_restart_window_open = true;
      }
    }
  };
  browseFolderCaller = 1;
  OpenFileSelector(ICON_FA_SD_CARD "  Change Data Folder", true, std::move(callback),
                   ImGuiFullscreen::FileSelectorFilters(), currentStorage);
}
...
void DrawLandingWindow()
{
  ...
  if (BeginFullscreenColumnWindow(0.0f, cend, "logo", ImVec4(0.11f, 0.15f, 0.17f, 1.00f)))
  {
    ...
    auto text_width = ImGui::CalcTextSize(title, nullptr, false, image_size).x;
    float text_fix = LayoutScale(70.0f);
    if (!isLandscape)
    {
      text_fix = -LayoutScale(30.0f);
    }
    ImGui::SetCursorPosX(text_fix + (image_size - text_width) / 2.0f);
  ...
  if (BeginFullscreenColumnWindow(cstart, LAYOUT_SCREEN_WIDTH, "menu"))
  {
    /*if (isLandscape) {
    BeginMenuButtons(8, 0.4f);
    }
    else */
    {
      BeginMenuButtons(8, 0.5f);
    }
    ...
    std::string currentStorage = getCurrentStorage();
    std::string storageHint = "Your duckstation data folder is (";
    storageHint.append((currentStorage.empty() ? "LocalFolder)" : "Custom)"));
    if (MenuButton(" " ICON_FA_SD_CARD "  Storage", storageHint.c_str()))
      s_host_interface->RunLater(ChangeDataFolder);
...
bool HandleKeyboardBinding(int code, bool pressed)
{
  ...
  auto keyName = GetKeyCodeName(code);
  if (keyName != nullptr)
  {
    TinyString value;
    value.Format("Keyboard/%s", keyName);
...
{
  browseFolderCaller = 5;
  std::string currentStorage = bios_directory;
  if (currentStorage.empty())
  {
...
{
  browseFolderCaller = 6;
  std::string currentStorage = memory_card_directory;
  if (currentStorage.empty())
  {
    currentStorage = getCurrentStorage();
    if (currentStorage.empty())
    ...
    currentStorage.append("\\memcards");
  }
  OpenFileSelector(
    "Memory Card Directory", true,
    [](const std::string& path) {
      if (!path.empty())
      {
...
void DrawSettingsWindow()
{
                            ...
                            ImVec4(0.18f, 0.18f, 0.18f, 1.00f)))
  {
    ...
    static constexpr std::array<const char*, static_cast<u32>(SettingsPage::Count)> titles2 = {
      {"", "", "", "", "", "", "", "", "", "", "", "", ""}};
    ...
    if (!isLandscape)
    {
      NavTitle(titles2[static_cast<u32>(s_settings_page)]);

      RightAlignNavButtons(static_cast<u32>(titles2.size()), ITEM_WIDTH, LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY);
      for (u32 i = 0; i < static_cast<u32>(titles2.size()); i++)
      {
        ...
        "settings_parent"))
  {
    ...
    switch (s_settings_page)
    {
      case SettingsPage::InterfaceSettings:
      {
        ...
        MenuHeading("Compatibility");

        // if (!isLevel93)
        {

          {
            /*settings_changed |= ToggleButton("D3D FL 9.3", "Force DirectX feature level 9.3 mode (for testing only).",
                                             &s_settings_copy.d3d93_compatibility);*/
            if (forcedByDeviceCreate &&
                (s_settings_copy.shader_compatibility != 0 && s_settings_copy.shader_compatibility != outLevelIndex))
            {
              ActiveButton(ICON_FA_EXCLAMATION_TRIANGLE
                           "  Shader level set to Auto due to current selection is not compatible.",
                           false, false, ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY);
            }

            static const auto shader_levels = [](u32 value) -> const char* {
              static constexpr std::array<const char*, 5> texts = {
                {"Auto", "Level 9", "Level 10", "Level 11", "Level 12"}};
              return (value >= texts.size()) ? "" : texts[value];
            };
            settings_changed |=
              EnumChoiceButton<u32, u32>("Shader Level", "Older devices require lower level (restart required)",
                                         &s_settings_copy.shader_compatibility, shader_levels, 5);

            static const auto shader_optimization = [](u32 value) -> const char* {
              static constexpr std::array<const char*, 5> texts = {
                {"Auto", "Level 3 (Performance)", "Level 2 (Balanced)", "Level 1 (Fast Compiling)", "Level 0 (Off)"}};
              return (value >= texts.size()) ? "" : texts[value];
            };
            settings_changed |=
              EnumChoiceButton<u32, u32>("Shader Optimization", "Higher level better performance, slower compiling",
                                         &s_settings_copy.shader_opt, shader_optimization, 5);

            settings_changed |=
              ToggleButton("Shader Cache", "Turn it off if you have render issues.", &s_settings_copy.cache_shader);
          }
          if (isLevel93)
          {
            settings_changed |= ToggleButton("Compatibility Notice", "Show notice at startup if there is a problem.",
                                             &s_settings_copy.d3dnotice);
          }
          ...
          MenuHeading("Speed");
          settings_changed |=
            RangeButton("Buffer Count", "Lower for low latency, higher better but memory consuming (restart required).",
                        &s_settings_copy.buffer_count, 2, 12, 1, "%d");
          settings_changed |= RangeButton(
            "FPS Limit", "Limits how many frames are displayed, combain between Limit and Max Frames for better speed.",
            &s_settings_copy.display_max_fps, 5.0f, 120.0f, 1.0f, "%.2f FPS");
          settings_changed |= RangeButton("Max Frames", "Max frames to run in a single loop, depends on device speed.",
                                          &s_settings_copy.max_frames_to_run, 1, 6, 1, "%d Frame");
#ifdef _WIN32
          settings_changed |=
          ...
          if (RangeButton("Overclocking Percentage",
                          "The normal clock speed, reduce this for old devices (85%-90%).",
                          &overclock_percent, 10, 200, 1, "%d%%", s_settings_copy.cpu_overclock_enable))
          {
        ...
        {
          settings_changed |= ToggleButton("Confirm Power Off",
                                           "Confirm shutting down the emulator/game "
                                           "when the hotkey is pressed.",
                                           &s_settings_copy.confim_power_off);
        }
        if (isLandscape)
        ...
        {
          settings_changed |= ToggleButton("Save State On Exit",
                                           "Automatically saves the emulator state when stop. You can then "
                                           "resume directly next time.",
                                           &s_settings_copy.save_state_on_exit);
        }
        settings_changed |=
      ...
      case SettingsPage::GameListSettings:
      {
        ...
        {
          auto previewPath = GetPreviewPath(entry.path);
          MenuButton(previewPath.c_str(), entry.recursive ? "Scanning Subdirectories" : "Not Scanning Subdirectories",
                     false);
        }
...
#undef MAKE_EMULATION_SPEED
      ...
      case SettingsPage::BIOSSettings:
      {
        ...
        auto previewDPath = GetPreviewPath(bios_directory);
        if (MenuButton("BIOS Directory", previewDPath.c_str()))
        {
          DoBIOSDirectory();
        }
      ...
      case SettingsPage::ControllerSettings:
      {
        ...
        if (AccelerometerReady)
        {
          MenuHeading("Sensors");
          settings_changed |=
            ToggleButton("Enable", "Enable sensors (Left, Right by default)", &s_settings_copy.sensors_enable);
          if (s_settings_copy.sensors_enable)
          {
            settings_changed |= ToggleButton("Tilt Up", "Invoke Up when device tilted up", &s_settings_copy.sensors_up);
            settings_changed |=
              ToggleButton("Tilt Down", "Invoke Down when device tilted down", &s_settings_copy.sensors_down);
            settings_changed |= RangeButton("Sensitivity", "Increase value to reduce the sensitivity",
                                            &s_settings_copy.sensors_sens, 0.1f, 5.0f, 0.01f, "%.2f");
          }
        }
        MenuHeading("Touch Controller");

        settings_changed |= ToggleButton("Enable", "Enable on-screen touch controller", &s_settings_copy.touch_enable);

        if (s_settings_copy.touch_enable)
        {
          settings_changed |= RangeButton("Touch Port", "Select which port should touch controls report to",
                                          &s_settings_copy.touch_port, 1, NUM_CONTROLLER_AND_CARD_PORTS, 1, "Port %d");
          settings_changed |=
            ToggleButton("Show Always", "Prevent touch controller from hide if gamepad/keyboard detected.",
                         &s_settings_copy.touch_force);

          settings_changed |= ToggleButton("Action buttons (Sensitivity)",
                                           "Extend sensitivity area for action buttons (circle, cross..et).",
                                           &s_settings_copy.touch_action_extra);

          settings_changed |=
            ToggleButton("DPad buttons (Sensitivity)", "Extend sensitivity area for DPad buttons (Up, Left..etc).",
                         &s_settings_copy.touch_dpad_extra);

          settings_changed |= RangeButton("Controls Opacity", "Adjust touch controls opacity.",
                                          &s_settings_copy.touch_opacity, 0, 80, 1, "%d%%");

          settings_changed |=
            RangeButton("Controls Scale (Landscape)", "Increase controls scale size on Landscape mode",
                        &s_settings_copy.touch_scale, 0.1f, 3.0f, 0.01f, "%.2f");

          settings_changed |= RangeButton("Controls Scale (Portrait)", "Increase controls scale size on Portrait mode",
                                          &s_settings_copy.touch_scalep, 0.1f, 3.0f, 0.01f, "%.2f");

          MenuHeading("Touch Analog");

          settings_changed |=
            ToggleButton("Enable Analog", "Show left analog stick, you need to change controller type to analog.",
                         &s_settings_copy.touch_analog);
          if (s_settings_copy.touch_analog)
          {
            settings_changed |= RangeButton("Analog (Sensitivity)", "More to increase the speed, less to reduce",
                                            &s_settings_copy.touch_analog_s, 0.1f, 3.0f, 0.01f, "%.2f");

            settings_changed |= ToggleButton("Hide DPad", "Hide DPad and place the analog in it's position.",
                                             &s_settings_copy.touch_hide_dpad);
            if (s_settings_copy.touch_hide_dpad)
            {
              settings_changed |=
                RangeButton("Analog Scale", "Increase analog scale size (only active when DPad is hidden)",
                            &s_settings_copy.touch_analog_sc, 0.1f, 3.0f, 0.01f, "%.2f");
            }
            settings_changed |= ToggleButton("Show L3-R3", "Show L3 R3 Buttons.", &s_settings_copy.touch_show_lr3);
          }
        }
#endif

        MenuHeading("GamePad Listeners");
        ActiveButton(ICON_FA_EXCLAMATION_TRIANGLE
                     "  Only if XInput failed, enabling with XInput can cause multiple calls.",
                     false, false, ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY);
        settings_changed |=
          ToggleButton("Gaming.Input Event", "Use Gaming.Input event, enable if GamePad is not detected",
                       &s_settings_copy.gaming_event);

        if (s_settings_copy.gaming_event)
        {
          settings_changed |= ToggleButton("UI Events (Input)", "Send gamepad input to the UI if no game started",
                                           &s_settings_copy.gaming_ui);
        }

        settings_changed |= ToggleButton("Native Event", "Use native key event, use it if Gaming.Input failed",
                                         &s_settings_copy.native_event);

        if (s_settings_copy.native_event)
        {
          settings_changed |= RangeButton("Native Port", "Choose which port should the event report to",
                                          &s_settings_copy.native_port, 1, NUM_CONTROLLER_AND_CARD_PORTS, 1, "Port %d");
          settings_changed |= ToggleButton("UI Events (Native)", "Send native key to the UI if no game started",
                                           &s_settings_copy.native_ui);
        }

        if (s_settings_copy.gaming_event || s_settings_copy.native_event)
        {
          settings_changed |= ToggleButton("Quick Menu Shortcut", "Show quick menu if Start+Select pressed",
                                           &s_settings_copy.gaming_quick);

          MenuHeading("GamePad Map");

          settings_changed |= ToggleButton("Custom Map", "Customized map for Gaming.Input and Native events",
                                           &s_settings_copy.gaming_map);
          if (s_settings_copy.gaming_map)
          {
            static const auto psx_buttons = [](u32 value) -> const char* {
              static constexpr std::array<const char*, 16> texts = {
                {"A", "B", "X", "Y", "LeftShoulder", "RightShoulder", "Up", "Down", "Left", "Right", "LeftTrigger",
                 "RightTrigger", "LeftThumbstick", "RightThumbstick", "Menu", "View"}};
              return (value >= texts.size()) ? "" : texts[value];
            };
            settings_changed |= EnumChoiceButton<u32, u32>("Cross", "Custom map for Cross button.",
                                                           &s_settings_copy.gaming_custom_cross, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Circle", "Custom map for Circle button.",
                                                           &s_settings_copy.gaming_custom_circle, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Square", "Custom map for Square button.",
                                                           &s_settings_copy.gaming_custom_square, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Triangle", "Custom map for Triangle button.",
                                                           &s_settings_copy.gaming_custom_train, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("L1", "Custom map for L1 button.",
                                                           &s_settings_copy.gaming_custom_l1, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("R1", "Custom map for R1 button.",
                                                           &s_settings_copy.gaming_custom_r1, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Up", "Custom map for Up button.",
                                                           &s_settings_copy.gaming_custom_up, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Down", "Custom map for Down button.",
                                                           &s_settings_copy.gaming_custom_down, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Left", "Custom map for Left button.",
                                                           &s_settings_copy.gaming_custom_left, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Right", "Custom map for Right button.",
                                                           &s_settings_copy.gaming_custom_right, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("L2", "Custom map for L2 button.",
                                                           &s_settings_copy.gaming_custom_l2, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("R2", "Custom map for R2 button.",
                                                           &s_settings_copy.gaming_custom_r2, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("L3", "Custom map for L3 button.",
                                                           &s_settings_copy.gaming_custom_l3, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("R3", "Custom map for R3 button.",
                                                           &s_settings_copy.gaming_custom_r3, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Start", "Custom map for Start button.",
                                                           &s_settings_copy.gaming_custom_start, psx_buttons, 16);
            settings_changed |= EnumChoiceButton<u32, u32>("Select", "Custom map for Select button.",
                                                           &s_settings_copy.gaming_custom_select, psx_buttons, 16);
          }
        }
        ...
        {
          ActiveButton(ICON_FA_EXCLAMATION_TRIANGLE
                       "  Profiles not active on (Gaming.Input, Native) events, use custom map above.",
                       false, false, ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY);
        }
        ...
        for (u32 port = 0; port < NUM_CONTROLLER_AND_CARD_PORTS; port++)
        {
          ...
          {
            ActiveButton(ICON_FA_EXCLAMATION_TRIANGLE
                         "  Buttons map not active on (Gaming.Input, Native) events, use custom map above.",
                         false, false, ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY);
          }
      ...
      case SettingsPage::MemoryCardSettings:
      {
        ...
        settings_changed |= ToggleButton("Use Single Card For Sub-Images",
                                         "Using a multi-disc image (m3u/pbp) and per-game (title) memory cards, "
        ...
        auto previewMDPath = GetPreviewPath(memory_card_directory);
        if (MenuButton("Memory Card Directory", previewMDPath.c_str()))
        {
          DoMemCardDirectory();
        }
      ...
      case SettingsPage::DisplaySettings:
      {
        ...
        if (isLevel93)
        {
          if (justStarted && s_settings_copy.gpu_renderer != GPURenderer::Software)
          {
            s_settings_copy.gpu_renderer = GPURenderer::Software;
            justStarted = false;
            settings_changed = true;
          }

          ActiveButton(ICON_FA_EXCLAMATION_TRIANGLE "  Render will be Software each start, Hardware not supported.",
                       false, false, ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY);
        }
        settings_changed |=
        ...
        if (!s_settings_copy.IsUsingSoftwareRenderer())
        {
          if (isLandscape)
          {
            settings_changed |= ToggleButton("Use Software For Readbacks (Slow)",
                                             "Runs the software renderer in parallel for VRAM readbacks. ",
        ...
        if (isLandscape)
        {
          ...
          settings_changed |=
            ToggleButton("Sync To Host Refresh Rate",
        ...
        {
          settings_changed |= ToggleButton("Optimal Frame Pacing",
                                           "Ensures every frame generated is displayed. Disable if "
                                           "you are having speed or sound issues.",
                                           &s_settings_copy.display_all_frames);
        }
        ...
        settings_changed |=
          EnumChoiceButton("Downsampling",
                           "Downsamples image before display. Can improve "
        ...
        {
          settings_changed |= ToggleButton("Show Display FPS",
                                           "Shows the number of frames displayed per second by the system "
                                           "in the top-right corner of the display.",
                                           &s_settings_copy.display_show_vps);
        }
        settings_changed |= ToggleButton(
      ...
      case SettingsPage::EnhancementSettings:
      {
        ...
        {
          settings_changed |= ToggleButton("Disable Interlacing",
                                           "Some games can render in 480p this way, "
                                           "but others will break.",
                                           &s_settings_copy.gpu_disable_interlacing);
        }
        ...
        {
          settings_changed |= ToggleButton("Force NTSC Timings",
                                           "i.e. 60hz. Some PAL games will run at their \"normal\" "
                                           "speeds, while others will break.",
                                           &s_settings_copy.gpu_force_ntsc_timings);
        }
        settings_changed |=
        ...
        {
          settings_changed |=
            ToggleButton("Chroma Smoothing For 24-Bit Display",
                         "Smooths out blockyness between colour transitions in 24-bit content. Applies "
                         "to the hardware renderers.",
                         &s_settings_copy.gpu_24bit_chroma_smoothing);
        }
      ...
      case SettingsPage::AudioSettings:
      {
        ...
        {
          settings_changed |= ToggleButton("Mute CD Audio",
                                           "Forcibly mutes both CD-DA and XA audio from the CD-ROM. used to "
                                           "disable background music in some games.",
                                           &s_settings_copy.cdrom_mute_cd_audio);
        }
        ...
        {
          settings_changed |= ToggleButton("Sync To Output",
                                           "Throttles the emulation speed based"
                                           ". Enable to reduce the chances of crackling.",
                                           &s_settings_copy.audio_sync_enabled);
        }
        settings_changed |= ToggleButton(
          "Resampling",
      ...
      case SettingsPage::AchievementsSetings:
      {
        ...
        ActiveButton(ICON_FA_EXCLAMATION_TRIANGLE
                     "  For load state, turn off.. load, then switch on, otherwise it may crash.",
                     false, false, ImGuiFullscreen::LAYOUT_MENU_BUTTON_HEIGHT_NO_SUMMARY);
        ...
        {
          settings_changed |= ToggleButtonForNonSetting(ICON_FA_STETHOSCOPE "  Test Mode",
                                                        "DuckStation will assume all achievements are locked and not "
                                                        "send any unlock notifications to the server.",
                                                        "Cheevos", "TestMode", false);
          settings_changed |=
            ToggleButtonForNonSetting(ICON_FA_MEDAL "  Test Unofficial Achievements",
                                      "DuckStation will list achievements from unofficial sets. These "
                                      "achievements are not tracked by RetroAchievements.",
                                      "Cheevos", "UnofficialTestMode", false);
        }
        ...
        if (isLandscape)
        {
          if (ToggleButtonForNonSetting(
                ICON_FA_HARD_HAT "  Hardcore Mode",
        ...
        {
          if (ToggleButtonForNonSetting(ICON_FA_HARD_HAT "  Hardcore Mode (Challenge mode)",
                                        "Disables save state, cheats, but you receive double the achievement points.",
                                        "Cheevos", "ChallengeMode", false))
          {
            s_host_interface->RunLater([]() {
              if (!ConfirmChallengeModeEnable())
      ...
      case SettingsPage::AdvancedSettings:
      {
        ...
        settings_changed |=
          ToggleButton("Log Touch Events", "Log touch screen events, don't use other than debug purposes.",
                       &s_settings_copy.touch_log);
        ...
        MenuHeading("CPU Emulation (Broken!)");
#else
...
void DrawQuickMenu(MainWindowType type)
{
                            ...
                            ImGuiWindowFlags_NoBackground))
  {
    ...
    {
      if (g_settings.confim_power_off)
      {
        SmallString confirmation_message2("Do you want to (save) before exit?");
        SmallString confirmation_message("Are you sure you want to exit?");
        if (s_host_interface->ShouldSaveResumeState())
        {
          if (s_host_interface->ConfirmMessage(confirmation_message2))
          {
            s_host_interface->PowerOffSystem(true);
          }
          else
          {
            s_host_interface->PowerOffSystem(false);
          }
        }
        else
        {
          if (s_host_interface->ConfirmMessage(confirmation_message))
          {
            s_host_interface->PowerOffSystem(false);
          }
        }
...
void DrawSaveStateSelector(bool is_loading, bool fullscreen)
{
  ...
  else
  {
    ...
    {
      ImGui::SetNextWindowSize(LayoutScale(600.0f, 680.0f));
    }
    ImGui::SetNextWindowPos(ImGui::GetIO().DisplaySize * 0.5f, ImGuiCond_Always, ImVec2(0.5f, 0.5f));
...
void DrawGameListWindow()
{
  ...
  if (BeginFullscreenColumnWindow(cstart, LAYOUT_SCREEN_WIDTH, "game_list_entries"))
  {
  ...
  if (BeginFullscreenColumnWindow(0.0f, cend, "game_list_info", ImVec4(0.11f, 0.15f, 0.17f, 1.00f)))
  {
    ...
    else
    {
      ImGui::Image(selected_entry ? GetGameListCover(selected_entry)->GetHandle() :
                                    GetTextureForGameListEntryType(GameListEntryType::Count)->GetHandle(),
                   LayoutScale(ImVec2(150.0f, 150.0f)));
    }
    ...
    constexpr float start_x2 = 20.0f;
    float text_y = isLandscape ? 425 : 225;
    float text_width;
    SmallString text;
    ...
    {
      ImGui::SetCursorPos(LayoutScale(start_x2, text_y));
    }
...
void DrawAboutWindow()
{
                             ...
                             ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize))
  {
    ...
    // All credits goes to stenzek, I add my name so old hardware users don't bother him with something is not under his maintenance
    ImGui::TextWrapped("Created by stenzek, This port is maintained by Bashar Astifan, "
                       "it's not up-to date code so don't bother the original developer with any issue.");
    ImGui::NewLine();
    ...
    {
      s_host_interface->RunLater(
        []() { s_host_interface->ReportError("Go to https://github.com/basharast/DuckStation-UWP-ARM"); });
    }
    if (ActiveButton(ICON_FA_BUG "  Issue Tracker", false))
    {
      s_host_interface->RunLater(
        []() { s_host_interface->ReportError("Go to https://github.com/basharast/DuckStation-UWP-ARM/issues"); });
    }
    if (ActiveButton(ICON_FA_COMMENT "  Telegram Group", false))
    {
      s_host_interface->RunLater([]() { s_host_interface->ReportError("Go to https://t.me/Windows10Phone"); });
    }
...
{
  ImGui::SetNextWindowSize(LayoutScale(600.0f, 280.0f));
  ImGui::SetNextWindowPos(ImGui::GetIO().DisplaySize * 0.5f, ImGuiCond_Always, ImVec2(0.5f, 0.5f));
  ImGui::OpenPopup("Compatibility");

  ImGui::PushFont(g_large_font);
  ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, LayoutScale(10.0f));
  ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, LayoutScale(10.0f, 10.0f));

  if (ImGui::BeginPopupModal("Compatibility", &s_compatibility_window_open, ImGuiWindowFlags_NoResize))
  {
    ImGui::TextWrapped("Your device GPU is old:\n-Only software render (could be slow)\n-You cannot upscale "
                       "quality\n-Use Screen scale 350%% or less");
    ImGui::NewLine();
    ImGui::TextWrapped("To disable this message go to Setting -> Compatibility Notice.");
...
{
  ImGui::SetNextWindowSize(LayoutScale(600.0f, 210.0f));
  ImGui::SetNextWindowPos(ImGui::GetIO().DisplaySize * 0.5f, ImGuiCond_Always, ImVec2(0.5f, 0.5f));
  ...
  if (ImGui::BeginPopupModal("Restart required!", &s_restart_window_open,
                             ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize))
  {
    ImGui::TextWrapped("You need to restart DuckStation to apply changes,\n"
                       "Re-open the app after it will close.");
    ImGui::NewLine();

    BeginMenuButtons();
    if (ActiveButton(ICON_FA_STAR "  Restart", false))
    {
      AddDataToLocalSettings("duckData", expectedPath, true);
      ImGui::CloseCurrentPopup();
      s_restart_window_open = false;
...
void DrawDebugSystemMenu()
{
  ...
  if (ImGui::MenuItem("Stop", nullptr, false, system_enabled))
  {
    DoPowerOff();
    ClearImGuiFocus();
  }
  ...
  ImGui::Separator();
  if (ImGui::BeginMenu("Touch Controller", system_enabled))
  {
    if (ImGui::MenuItem("Show Alawys", nullptr, &g_settings.touch_force))
    {
      ClearImGuiFocus();
    }
    if (ImGui::MenuItem("Enable Analog", nullptr, &g_settings.touch_analog))
    {
      ClearImGuiFocus();
    }
    ImGui::EndMenu();
